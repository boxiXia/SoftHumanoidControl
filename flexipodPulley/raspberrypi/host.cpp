/*****************host.cpp******************/
// compile:
// sudo g++ -pthread -lwiringPi host.cpp
...
constexpr int JETSON_PORT = 32002;
constexpr int PC_PORT = 32003;
...
public:
	...
	float k[MOTOR_NUM];
	float t_speed[MOTOR_NUM];
	MSGPACK_DEFINE(robot_command,k,t_speed);
};
...
public:
	...
	float timestamps;
	MSGPACK_DEFINE(timestamps, joint_pos, joint_vel, joint_cur);
};
...
int main()
{
	...
	/******************************************************************/
	wiringPiSetup();
	constexpr int pwm_pin = 4;
	pinMode (pwm_pin, PWM_OUTPUT) ; 

	softPwmCreate(pwm_pin, 0, 100);// PWM setting
	...
	while (1)
	{
		...
		{
			// Speed of motors:
			msg_to_pc.joint_vel[j] = comm->vel[j];
			// Angle of motors:
			msg_to_pc.joint_pos[j] = comm->pos[j];
			// Current of motors:
			msg_to_pc.joint_cur[j] = comm->cur[j];
		}
		//printf("%f\n", msg_to_pc.joint_pos[0]);
		...
		// }
		//printf("\n");
		softPwmWrite(pwm_pin,t_speed[0]);
		//std::cout<<t_speed[0]<<std::endl;
		//softPwmWrite(pwm_pin,0);
		//pwmWrite (pwm_pin,t_speed[0]);
