...
// then : ls
#define HOST_DEV_SERIALNB		6582060			
...
float k[MOTOR_NUM]; // pid setting
float t_speed[MOTOR_NUM];

// Teensy->host communication data structure
// sizeof(ESCPID_comm)=64 to match USB 1.0 buffer size
typedef struct {
  float    pos[MOTOR_NUM];      // Motors rotation angle
  float    vel[MOTOR_NUM];     // Motors rad/s
  float	   cur[MOTOR_NUM];     // Motors torque
  float    timestamps;
} Teensycomm_struct_t;
